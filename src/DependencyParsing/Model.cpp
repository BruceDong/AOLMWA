    ...
    for(int i = 0; i < n; i++){
		for(int j = 0; j < n; j++){
            ...
            for(size_t i = 0 ; i < fea.size(); i++)
            {
                    ...
                    if(fid != -1)
                    {
                            vector<int>::iterator result = find( vfid.begin( ), vfid.end( ), fid ); //≤È’“3
                            if  ( result == vfid.end( ) )
                            {
                                    vfid.push_back(fid);
                                }
...
bool Model::savingFeatureWeights(int learnTimes, int parts)
{
        ...
        {
                globalWeight[parts].clear();
		for(size_t i = 0; i < fWeight.size(); i++)
		{
			globalWeight[parts][i] = fWeight[i];
		}
        }
...
std::vector<double> Model::mergeFeatureWeight(int learnTimes, double senNum)
{
        if(learnTimes == 0)
        {
